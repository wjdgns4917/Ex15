...
public class FileEx {
	...
	for (int i = 0; i < subFiles.length; i++) { // subFiles배열의 각File에 대해 루프
		File f=subFiles[i];
	...
	public static void main(String[] args) {
		...
		String res="";
		if(f1.isFile())res="파일"; //파일 타입
		else if(f1.isDirectory())res="디텍토리"; //디렉토리 타입
 		System.out.println(f1.getPath()+"은 "+res+"입니다.");
		
		File f2=new File("c:\\Temp\\java_sample"); // 새로 만들고자하는 디렉토리
		if (!f2.exists()) { //f2 디렉토리가 존재하는지 검사
			f2.mkdir(); //존재하지 않으면 디렉토리 생성
		}
		listDirectory(new File("c:\\Temp")); //c:\Temp에 있는 파일리스트 출력
		f2.renameTo(new File("c:\\Temp\\javasample")); //java_sample->javasample로 이름변경
		listDirectory(new File("c:\\Temp")); //javasample로 변경한후 리스트 출력
	}
}
